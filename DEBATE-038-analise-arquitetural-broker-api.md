# Análise Arquitetural — Camada de API para o Central Broker
## Orchestrator HTTP vs NestJS direto

**Autor:** Claude (Arquiteto)
**Data:** 2026-05-31
**Referência:** DEBATE-038 · thread: arquitetura-balcao-central · pai: DEBATE-037

---

## Sumário Executivo

O Central Broker (SQLite, `tasks.db`) está operacional. O próximo passo é expô-lo ao Hive UI.
Duas opções foram avaliadas: adicionar um servidor HTTP ao Orchestrator (daemon) ou fazer o
backend NestJS acessar o SQLite diretamente em modo readonly.

**Decisão:** ✅ Opção B — NestJS acessa SQLite diretamente, com cinco condições obrigatórias.
...
### Opção B — NestJS acessa SQLite diretamente *(aprovada)*
...
> O Hive é um sistema fechado. O Central Broker é interno ao Hive.(Isso e uma verdade)
Não há usuário externo consumindo a API do Broker. Isso justifica SQLite sobre o (isso tambem e verdade)
> PostgreSQL hoje: não há escala multi-tenant, só um squad. (isso tambem e verdade)

---

### 3.2 C4 — Nível 2: Containers

Como o Hive está organizado internamente em processos e artefatos.
...
| Migração PostgreSQL | Dois pontos de migração | Um único repositório a trocar |
...
O `BrokerService` depende da interface, não da implementação concreta. Quando a migração para
PostgreSQL estiver madura, apenas a implementação troca — zero impacto em controllers e
...
// Implementação futura (PostgreSQL/Prisma)
@Injectable()
...
### 3.10 Alinhamento com migração futura para PostgreSQL

A Opção B foi desenhada para não travar a futura migração de banco.

```mermaid
sequenceDiagram
    participant NOW as Hoje
    participant THEN as Futuro (PostgreSQL)
    ...
    NOW->>CODE: providers: [{ provide: TaskRepository, useClass: SqliteTaskRepository }]
...
A Opção A exigiria migrar dois pontos: o Orchestrator E a API HTTP que o NestJS consome —
risco duplo, custo duplo.
...
| DB path hardcoded quebra em novo ambiente | Média | Médio | C3: `HIVE_TASKS_DB_PATH` no `config.env` — único lugar a mudar |
| Migração PostgreSQL não lembra do NestJS | Baixa | Alto | C4: `TaskRepository` isola — trocar implementação não afeta nada acima |
| Leitura retorna snapshot anterior ao último commit | Alta | Baixo | Comportamento correto do WAL; polling/WS corrige em < 1s |
...
| Item | Valor |
|---|---|
| **Custo estimado** | R$ 80–120 (1 WO média, ~4–6h de implementação) |
| **Confiança** | Alta — `tasks-json.js` já valida o padrão readonly; `better-sqlite3` já é dependência do Orchestrator |
| **Valor gerado** | Elimina ~50-100ms por poll; remove acoplamento cruzado de `node_modules`; abre caminho para Broker Dashboard em tempo real; base isolada para migração PostgreSQL |
| **Payback** | Imediato — melhora performance e manutenibilidade na primeira entrega |
| **Custo de não fazer** | Débito técnico no `execFile` cresce com novos endpoints; impossibilita escrita via UI sem gambiarra; bloqueia Broker Dashboard como feature |

---

## 5. Próximos passos

1. Validação pelos demais stakeholders (Gemini — ROI; Copilot — viabilidade)
2. Aprovação no Gate
3. Blueprint técnico do `BrokerModule` com contrato completo de endpoints
4. WO de implementação:
   ...
   - Endpoints `GET /api/broker/tasks` e `GET /api/broker/tasks/:id`
5. WO secundária: externalizar `HIVE_TASKS_DB_PATH` para `config.env`

---

*Documento gerado a partir do DEBATE-038 · beehive/construcao/debates/DEBATE-038-BROKER-API-CAMADA-ACESSO.md*
