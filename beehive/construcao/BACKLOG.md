...
- **DT-008** — `sqlite-task-store.ts`: `failTask(id, reason?)` descarta `reason` com `void reason` e o schema não tem coluna `fail_reason TEXT`. Falhas ficam opacas — sabe-se que uma task falhou, mas não por quê. Registrado em: Auditoria DEBATE-037 (2026-05-31). Impacto: baixo — correto funcionalmente, mas compromete diagnóstico retrospectivo. Corrigir após Fase 2 estabilizar.
- **DT-009** — `scripts/tasks-json.js:22`: `db.pragma('busy_timeout = 3000')` diverge do padrão `5000ms` fixado em RC-001 (Auditoria DEBATE-037 § 9). Todos os outros consumidores do `tasks.db` usam 5000. Registrado em: Auditoria WO-048 (2026-05-31). Impacto: baixo — risco de timeout prematuro sob carga alta; alinhar na próxima revisão de `scripts/`.
- **DT-010** — `hive.service.ts` — `readCopilotDetailFromTasks`: filtra apenas `domain === 'hive'` para derivar `inboxPending` do Copilot. Tasks com `domain: 'shared'` atribuídas ao Copilot não são contadas, subestimando a carga real. Registrado em: Auditoria WO-048 (2026-05-31). Impacto: baixo — sem tasks `shared` ativas atualmente; corrigir quando domínio `shared` for utilizado.
